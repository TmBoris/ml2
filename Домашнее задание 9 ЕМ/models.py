...
class WordAligner(BaseAligner):
    ...
    def _e_step(self, parallel_corpus: List[TokenizedSentencePair]) -> List[np.array]:
        ...
        for sent_pair in parallel_corpus:
            q = self.translation_probs[np.ix_(sent_pair.source_tokens, sent_pair.target_tokens)]
            corpus_qs.append(q / q.sum(axis=0))
    ...
    def _compute_elbo(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]) -> float:
        ...
        ans = 0
        eps = 1e-8
        for sent_pair, posterior in zip(parallel_corpus, posteriors):
            ans += np.sum(posterior * 
                          (np.log(eps + self.translation_probs[np.ix_(sent_pair.source_tokens, sent_pair.target_tokens)]) 
                           - np.log(len(sent_pair.source_tokens) * (posterior + eps))))
    ...
    def _m_step(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]):
        ...
        for sent_pair, posterior in zip(parallel_corpus, posteriors):
            np.add.at(self.translation_probs, np.ix_(sent_pair.source_tokens, sent_pair.target_tokens), posterior)
    ...
    def align(self, sentences):
        ...
        for sentence in sentences:
            ...
            ans.append(list(zip(source_tokens, np.arange(1, len(sentence.target_tokens) + 1, dtype=np.int32))))
        return ans
